...
- [x] 검증 프롬프트 개선 — 사진 캡션 오판 방지 규칙 추가, 81개 테스트 통과 (2026-03-02)
- [x] 피드백 채널 추가 — footer에 "의견 보내기" Google Form 링크, 81개 테스트 통과 (2026-03-02)
- [x] 검증 오분류 방지 — misleading 판정 기준 강화 (프롬프트 3섹션 재구성), 81개 테스트 통과, workflow_dispatch 검증 완료 (2026-03-02)
...
- 2026-03-02: **검증 프롬프트 개선 — 사진 캡션 오판 방지**
  - `VERIFY_PROMPT_TEMPLATE` 판단 기준에 사진 캡션 제외 규칙 추가
  - 한국 뉴스의 자료사진 관행 반영 — 캡션 날짜/장소 불일치만으로 misleading 판정하지 않도록
  - 실제 사례: 이란-미 항모 기사에서 2025년 자료사진 캡션 때문에 ❌왜곡 오판 발생
- 2026-03-02: **피드백 채널 추가**
  - footer에 "의견 보내기" 버튼 추가 → Google Form 연결
  - 후원 링크 아래 회색 테두리 버튼으로 배치 (후원보다 낮은 시각적 우선순위)
  - `.footer-actions` flex 컨테이너로 후원 + 피드백 링크 정렬
- 2026-03-02: **검증 오분류 방지 — misleading 판정 기준 강화**
  - `VERIFY_PROMPT_TEMPLATE` 판단 기준 섹션을 3개 섹션으로 재구성
  - `## 중요: "확인 안 됨"과 "사실과 다름"은 완전히 다릅니다` — 별도 경고 섹션 신설
  - `## 판단 기준` — misleading 정의를 "명백히 모순되는 공식 출처가 있을 때만"으로 좁힘, 부분 미확인 기준 명시
...
4. 랜딩페이지 제작 — 취소 결정정
5. ~~유저 피드백 채널~~ ✅ — Google Form으로 구현 완료
6. 재방문(Sticky) 성장 전략
7. 브리핑 하루 2회 실행 검토 — 현재 KST 07:00 1회만 실행하므로 오전 7시 이후 속보 누락 가능. Google News RSS는 요청 시점 기준 실시간 동적 생성이라 피드 자체의 누락은 없지만, 파이프라인 실행이 1회뿐인 게 구조적 한계. 필요 시 오후 실행(예: 18:00) 추가
8. 스크래핑 전처리 강화 — article_text에서 사진 캡션/면책조항/저작권 문구를 사전 제거하여 AI 혼동 방지
9. 검증 2차 확인 — misleading 판정 시 자동 재검증(프롬프트 변형 or 다른 검색어) 후 2회 일치만 확정
10. reason 기반 자동 보정 — reason 텍스트에 "확인되지 않" 포함인데 tag이 misleading이면 자동으로 unconfirmed 보정
11. 검증 결과 로깅 — tag 분포, evidence 수, reason 키워드를 구조화 로그로 기록하여 오분류 패턴 모니터링